...
air_sf %>% distinct(SamplingPoint)
plot_daily <- daily_air %>% filter(day>=as.POSIXct("2024-05-15")&day<=as.POSIXct("2024-05-25"))
plot_daily_sodaq <- sodaq_daily %>% filter((day>=as.POSIXct("2024-05-15"))&(day<=as.POSIXct("2024-05-25")))

```
We need to prep it for the INLA model. I need to extract the raster values for every station location and time point.
# TODO: 1-21: Extract station locations from raster and at each time point.
```{r rast-process}
library(exactextractr)
station_coords <- st_coordinates(air_sf)
set_station_loc <- as.data.frame(unique(station_coords))
loc_sf <- st_as_sf(set_station_loc, coords = c("X", "Y"), crs = st_crs(air_sf)) 
loc_sf
ggplot() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme_minimal()
no2_loc_extract <- exact_extract(no2_rast, loc_sf, "mean", progress=F)
```
