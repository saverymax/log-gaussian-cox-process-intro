...
Having covered the need-to-know material for INLA and SPDE, next I'm going to build a spatial model using both ```INLA``` and ```inlabru```. Comparing the two packages is useful because, though they can be expected to give the same results in most cases, the implementation is a bit different and knowing when using the more complicated ```INLA``` can be justified is a useful exercise I think. 

I found https://becarioprecario.bitbucket.io/spde-gitbook/ch-lcox.html and https://becarioprecario.bitbucket.io/inla-gitbook/ch-INLA.html good for details about inla and such specifications, but more clear in general. The example for inla is from Moraga.

This initial part follows that of the book of Moraga <https://www.paulamoraga.com/book-spatial/sec-geostatisticaldataSPDE.html>.
