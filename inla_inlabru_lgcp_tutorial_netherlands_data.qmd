...
library(rasterVis)
```
Now for the data. I'll use butterfly data from citizen science observations made in the Netherlands in 2023. The data is available in the GBIF repository <https://doi.org/10.15468/dl.p5cy6n>, which includes all butterfly observations uploaded to observation.org. So it's necessary to filter the dataset down to just the Netherlands. I filter for the species Lasiommata megera. Here is a nice picture (Charles J. Sharp - Own work, from Sharp Photography, sharpphotography.co.uk, CC BY-SA 4.0, <https://commons.wikimedia.org/w/index.php?curid=81510351>):
...
```
`prec = 0.001^2` sets a vague gaussian prior, $N(0, \sigma^2=1/\text{prec}=1000)$.
...
We have to make sure to get the domain for the sampler correct. The INLA code does it above, but inlabru by default does not. More about that here <https://inlabru-org.github.io/inlabru/articles/2d_lgcp_plotsampling.html>
...
summary(fit1)
```
We can again make predictions across the domain, using the generated prediction points.  
```{r preds-intensity}
