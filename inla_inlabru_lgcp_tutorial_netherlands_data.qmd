...
summary(fit1)
```
$\beta_0$ is -17, which is quite low because the average baseline intensity $n_{obs}/\text{Area_D}$ is expected to be very small. This is because there are not that many observations (2705) over the whole of the Netherlands, especially not per/meter.
