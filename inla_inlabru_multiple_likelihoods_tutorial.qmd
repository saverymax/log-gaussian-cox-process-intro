...
rmatern_sim <- function(coords, kappa, variance, lambda=1){
  ...
  c(chol(Sigma) %*% rnorm(n=n, mean=1, sd=1))
}
...
kappa_u <- 7
sigma2_u <- 0.3
u <- rmatern_sim(loc_y, kappa_u, sigma2_u)
u
length(u)
set.seed(233)
...
# One question for my own data: whta to set priors to?
#  prior.range = c(0.1, 0.01),
#  prior.sigma = c(1, 0.01)
#)
# Default is gaussian so we don't need to name it but useful to see.
gaus.prior <-  list(prior = 'gaussian', param = c(-1, 2))
...
precision_prior <- list(theta=list(prior = 'loggamma', param=c(1,0.1)))

fit_multi_lik_inlabru <- bru(shared_component, lik_x, lik_y,
  options = list(
    control.family=list(list(hyper=precision_prior), list(hyper=precision_prior)),
    control.inla = list(
      strategy = "simplified.lapplace",
      int.strategy = "eb")
    )
  )
```
...
TODO, 12-16: Just messing with the priors and trying to figure out why precision is so badly estimated. A bit hard to tell actually. I messed around with the priors and that fixed some things and changed outhers.
And put my own data in.
```{r check-fit}
